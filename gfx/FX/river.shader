...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT Input ) : PDX_COLOR
		{
		...
		#ifdef LOW_END_GFX
			float4 diffuseColor = float4( waterColor, 0.5f );
			...
			float3 reflectiveColor = texCUBE( ReflectionCubeMap, reflection ).rgb * 0.8;
			...
			float vOpacity = 1.0f - cam_distance( GB_CAM_MIN, ( GB_CAM_MAX + 100 ) );
